...
def api(path: str):
    headers = {
        ...
        "User-Agent": "joe-command-profile",
...
def queue_block(snaps: list[dict]) -> str:
    ...
    for s in snaps:
        for pr in s["pulls"]:
            work.append((pr.get("updated_at") or "", "PR", s["repo"], pr.get("number"), clean(pr.get("title", ""), 66)))
        for issue in s["issues"]:
            work.append((issue.get("updated_at") or "", "ISSUE", s["repo"], issue.get("number"), clean(issue.get("title", ""), 66)))
...
def telemetry_block(snaps: list[dict]) -> str:
    ...
    for s in snaps:
        rows.append(
            f"| {s['label']} | `{s['branch']}` | `{s['sha']}` | {ago(s['last_date'])} | "
...
def render_live_svg(snaps: list[dict]) -> None:
    ...
    for i, (label, value) in enumerate(metrics):
        x = 32 + i * 190
        cards.append(
            f'<rect x="{x}" y="116" width="170" height="88" rx="10" fill="{CARD}" stroke="{BORDER}"/>'
            f'<text x="{x+14}" y="143" fill="{CYAN}" font-size="12" font-family="monospace">{html.escape(str(label))}</text>'
            f'<text x="{x+14}" y="182" fill="{WHITE}" font-size="30" font-weight="700" font-family="monospace">{html.escape(str(value))}</text>'
        )
    lines = []
    y = 252
    for s in snaps:
        ...
        c = color(sig)
        lines.append(
            f'<circle cx="48" cy="{y-5}" r="5" fill="{c}"/>'
...
<text x="32" y="48" fill="{GREEN}" font-size="24" font-weight="700" font-family="monospace">JOE // COMMAND</text>
...
def main() -> None:
    ...
    text = text.replace("JOE // COMMAND V5.2", "JOE // COMMAND").replace("JOE // COMMAND V5", "JOE // COMMAND")
    README.write_text(text)
    render_live_svg(snaps)
    print("JOE // COMMAND terminal-green profile updated")
